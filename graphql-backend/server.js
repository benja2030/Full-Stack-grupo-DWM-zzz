// Importamos el paquete express. Es el framework utilizado para construir el servidor web
const express = require('express');
// Importamos mongoose. Lo utilizamos para traducir nuestro codigo a querys para mongoDB
const mongoose = require('mongoose');
// Importa body-parser. Traduce lo que entrega el servidor a algo que pueda leer el browser
const bodyParser = require('body-parser');
// Importa cors. Para seguridad del servidor
const cors = require('cors');
// importamos modulos Apollo Server y GraphQL.
const {ApolloServer, gql} = require('apollo-server-express');
// Importa lodash. Ayuda el trabajo con javascript
...
// Conectamos a la base de datos en mongoDB atlas
mongoose.connect('mongodb+srv://benja2018:benja2020@cluster1.hfeg7i8.mongodb.net/bdwebmovil3', 
...
type Mutation{
    ...
    addCarroProductos(id: ID!, input: CarroInputProductos): Carro
    deleteCarroProductos(id: ID!, input: CarroInputProductos): Carro
    updateUsuario(id: ID!, input: UsuarioInput): Usuario
    updatePerfil(id: ID!, input: PerfilInput): Perfil
    deletePerfilUsuario(id: ID!, input: PerfilInput): Perfil
    updateProducto(id: ID!, input: ProductoInput): Producto
    updateCategoria(id: ID!, input: CategoriaInput): Categoria
    deleteCategoriaProducto(id: ID!, input: CategoriaInput): Categoria
...
const resolvers = {
    Query: {
        ...
        async getPerfil(obj,{id}){
            //const perfil = await Perfil.findById(id);
            const perfil = await Perfil.findById().
            populate({path: 'idUsuarios', populate: {path: 'idUsuarios'}})
            return perfil
        },
    ...
    Mutation: {
        ...
        async addUsuario(obj, { input }){
            ...
            let {nombre, direccion, email, verEmail, pass, telefono, sexo, perfilString} = input;
            perfilString = "632ddb16a89a48f4e6375cb9"
            ...
            if(perfilB === null){
                ...
                usuario = new Usuario({nombre:nombre, direccion:direccion, email: email, verEmail: 0, pass: pass, 
                                    telefono:telefono, sexo:sexo, fechaCreacion:fecha});
                await usuario.save();
            }else{
                // Crea el usuario
                usuario = new Usuario({nombre:nombre, direccion:direccion, email: email, verEmail: 0,
                    pass: pass, telefono:telefono, sexo:sexo, idPerfil: perfilB._id, fechaCreacion:fecha});
                await usuario.save();
                console.log(perfilB._id)
                console.log(usuario._id)
                await Perfil.updateOne(
                    {_id: perfilB._id},
                    {$push: {idUsuarios: usuario._id}}
                    );
                console.log("cac2")
            }
            return usuario;
        ...
        async updatePerfil(obj, { id, input }){
            ...
            let {usuarioString, nombre, descripcion} = input;
            // Busca el usuario por su ID
            let usuarioB = await Usuario.findById(usuarioString);
            let perfil
            if(usuarioB === null){
                // Actualiza solo el nombre y la descripcion
                perfil = Perfil.findByIdAndUpdate(id, {nombre: nombre, descripcion: descripcion});
            }
            else{
                // Busca perfil
        ...
        async deletePerfilUsuario(obj, { id, input }){
            // Guarda el string del id del usuario y su nombre
            ...
            if(usuarioB === null){
                ...
                if(perfil.idUsuarios.includes(usuarioB._id)){
                    console.log("entra")
                    perfil.idUsuarios.pop(usuarioB._id)
                    await perfil.save()
                }else{
        ...
        async addProducto(obj, {input}){
            ...
            let {llave, nombre, precio, imagen, descripcion} = input;
            ...
            if(llave == null){
                ...
                producto = new producto({nombre: nombre, precio: precio, imagen: imagen, descripcion: descripcion});
                await producto.save();
            }
            else{
                // Agrega todo
                producto = new Producto({llave:llave, nombre: nombre, precio: precio, imagen: imagen, descripcion: descripcion});
                await producto.save();
            }
            return producto;
        },
        ...
        async updateProducto(obj, {id, input}){
            ...
            if(llave === null){
                producto = await Producto.findByIdAndUpdate(id, {nombre: nombre, precio: precio, imagen: imagen, descripcion: descripcion});
            }else{
                // Agrega todo
        ...
        // Borra un producto de una categoria por ID
        async deleteCategoriaProducto(obj, { id, input }){
            // Guarda input en variables
            ...
            if(productoB === null){
                ...
                // Revisa si id ya existe
                if(categoria.idProductos.includes(productoB._id)){
                    categoria.idProductos.pop(productoB._id)
                    await categoria.save()
                }else{
        ...
        // Agrega los productos del carro
        async addCarroProductos(obj, { id, input }){
        ...
        async deleteCarroProductos(obj, { id, input }){
            // Guarda el string del id del producto del menu que se quiere borrar
            let {menuString} = input;
            // Busca el producto en el menu por id
            let menuB = await Menu.findById(menuString);
            let carro
            ...
            if(menuB === null){
                ...
                // Busca carro
                carro = await Carro.findById(id);
                // Pregunta si el producto esta en el carro
                if (carro.idProductos.includes(menuB.idProducto)){
                    // Elimina el producto del carro
                    carro.idProductos.pop(menuB.idProducto);
                } else{
                    console.log("No esta el producto")
                }
                console.log(carro.idProductos);
                
                // Resta precio producto a total
                carro.total -= menuB.precio;
                await carro.save();
            }
            return carro;
